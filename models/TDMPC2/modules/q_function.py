...
"""
Methods from the original implementation's WorldModel class transformed into separate classes
"""
...
class QFunction(nn.Module):
	...
	def __init__(self, cfg):
		...
		self.q_target = QTarget(self._Qs)
...
class QTarget(nn.Module):
	...
	Can be used for both single-task and multi-task experiments.
	"""
	def __init__(self, cfg, Qs):
		super().__init__()
		self.cfg = cfg
		self._target_Qs = deepcopy(Qs).requires_grad_(False)
	

	def forward(self, z, a, return_type='min'):
		"""
		Predict state-action value.
		`return_type` can be one of [`min`, `avg`, `all`]:
			- `min`: return the minimum of two randomly subsampled Q-values.
...
class Policy(nn.Module):
	...
	def forward(self, z, task):
		...
		mu, log _std = self._pi(z).chunk(2, dim=-1)
