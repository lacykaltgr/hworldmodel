...
from common.distributions import unimix_logits
...
class RSSMPriorV3(nn.Module):
    ...
    def __init__(self, action_spec, hidden_dim=600, rnn_hidden_dim=600, state_vars=32, state_classes=32):
        ...
        self.rnn_to_prior_projector = nn.Sequential(
                ...
                nn.LayerNorm(hidden_dim),
                nn.SiLU(),
                nn.Linear(hidden_dim, state_vars * state_classes)   # TODO: initialize this to 1
        )
    ...
    def forward(self, state, belief, action):
        ...
        mixed_logits = unimix_logits(reshaped_logits, 0.01)
        dist = self.get_distribution(mixed_logits)
        state = dist.rsample()
        state = state.view(logits.shape)
        return mixed_logits, state, belief
...
class RSSMPosteriorV3(nn.Module):
    ...
    def __init__(self, hidden_dim=600, state_vars=32, state_classes=32):
        super(RSSMPosteriorV3, self).__init__()
