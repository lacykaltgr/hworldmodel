...
class RSSMPriorV2(nn.Module):
    ...
    def forward(self, state, belief, action):
        ...
        # if planning is used the shape of the input/state is (batch_size, n_trajectories, state_dim)
        # reshape it to (batch_size * n_trajectories, state_dim) so that the GRU can process it
        reshape_belief = None
        if belief.ndimension() == 3:
            reshape_belief = belief.shape
            belief = belief.view(-1, belief.shape[-1])
            action_state = action_state.view(-1, action_state.shape[-1])
            
            
        with torch.autocast(device_type='cuda', dtype=torch.float32):
            belief = self.rnn(action_state, belief)
        
        if reshape_belief is not None:
            belief = belief.view(reshape_belief)
