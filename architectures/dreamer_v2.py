    ...
    def _init_losses(self, config):
        ...
        losses = nn.ModuleDict(
            world_model = DreamerModelLoss(
                modules["world_model"]
            ).with_optimizer(params=modules["world_model"].parameters(), 
            ...
            actor = DreamerActorLoss(
                ...
                discount_loss=True
            ).with_optimizer(params=modules["actor_simulator"].parameters(), 
                             lr=config.optimization.actor_lr, weight_decay=1e-6),
            
            value = DreamerValueLoss(
                modules["value_model"],
                discount_loss=True
            ).with_optimizer(params=modules["value_model"].parameters(), 
