...
class CategoricalDist(SafeProbabilisticTensorDictSequential):
    ...
    def __init__(self,
        ...
        if spec is not None:
            param_spec = {"logits": UnboundedContinuousTensorSpec(spec.shape, spec.device)}
            params_spec = CompositeSpec(**param_spec)
            out_spec = CompositeSpec(**{out_key: spec})
        else:
            params_spec = None
            out_spec = None
        
        is_next = isinstance(out_key, tuple) and tuple[0] == "next"   
        inner_key = ["next_logits"] if is_next else ["logits"]
